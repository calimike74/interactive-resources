...
- **A-level, `data-stage=marked`:** a marks panel beside the paper. The question's stem, then every mark in the scheme's own wording with the year in the panel's head, ticked or not, each with what the bench measured beneath it; then the ladder, `T = 2.00 ms = 0.002 s   f = 1 ÷ T = 500 Hz`; and "as directed" or "not yet" in the corner. On the paper itself the bracket gains its number and an amplitude arrow appears from the centre line to the peak, which is what 2024 Q4(a) asks a candidate to label. A DC offset is drawn as a coral dashed line where the wave actually sits.
...
- `node scripts/check-bench.mjs http://localhost:PORT/squared-paper`: **all clear in Chromium and in WebKit**, at 1280×700 and 1440×900. Fixture `squared-paper`: presets 2023: an octave lower / 2025: louder, judge `Judge: the period kept` landing `data-verdict = period-kept`; stages `paper` / `marked` / `harmonics`.
