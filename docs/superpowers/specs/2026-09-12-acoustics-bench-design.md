...
- **A label centred on a marker will cross another marker sooner or later.** The masker's own label was centred under its peak, and with the masker Below the target it ran into the target's upright line at 1 kHz, which cut the "dB" off it. Both labels now choose a side: the masker's goes on the side of its peak the target is not on, and the target's "needs" readout moves to the other side of its own line when the masker's label has taken that row. Checked at both targets by all three masker places at two masker levels, twelve states.
