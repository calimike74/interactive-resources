...
**MCQ targets (12):** XLR identification/use · TRS vs TS conductor difference · RCA context (consumer/unbalanced) · which connector carries phantom power (XLR — per flashcards) · balanced rejection principle (inverted copy + difference) · long runs → balanced rule · DI box output (mic level, low impedance, balanced) · guitar-into-line-input fault diagnosis · why the limiter goes last in the chain · sensible chain order (EQ → compression → limiter per the course row) · which level tier a keyboard/line synth outputs · impedance mismatch symptom (thin/weak signal).
