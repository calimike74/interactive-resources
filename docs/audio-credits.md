...
The Synth bench (2026-09-01) plays no recording. Its subject is the oscillator, the one place the Bench Standard admits one (§3 law 5: "Oscillators may exist only where the subject IS the oscillator"), so the whole voice is built in the bench from OscillatorNodes (two per note, plus a square sub-oscillator when chosen) through a BiquadFilterNode and a GainNode the envelope drives, with one LFO OscillatorNode for the bench. `synthesis` is declared on the frame; law 9 still fails any bench that does not declare it. The four parts it plays (a bass, a pad, a lead, a keyboard part at 100 bpm in A minor) are note lists in `lib/bench/synth-model.js`, not files.
