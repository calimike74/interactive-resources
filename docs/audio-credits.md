...
## synth/ (no folder: the Synth bench is oscillators)

The Synth bench (2026-09-01; re-cut as the paper's panel 2026-09-02) plays no recording. Its subject is the oscillator, the one place the Bench Standard admits one (§3 law 5: "Oscillators may exist only where the subject IS the oscillator"), so the whole voice is built in the bench: per VCO one sawtooth OscillatorNode that serves both the saw and the pulse (the pulse is the saw minus itself through a DelayNode set to the width, which the LFO moves for PWM), a second VCO for a Pair or a Fifth, a square OscillatorNode as the sub-oscillator, and white noise as a two-second AudioBuffer of Math.random generated in the graph and looped (not a file), all through a BiquadFilterNode and a GainNode the envelope drives (or the gate, when the VCA says so), with one LFO OscillatorNode for the bench. `synthesis` is declared on the frame; law 9 still fails any bench that does not declare it. The four parts it plays (a bass, a pad, a lead, a keyboard part at 100 bpm in A minor) are note lists in `lib/bench/synth-model.js`, not files. From 2 Sep PM the wave slider has a shape switch (Saw, Tri, Sine), so each VCO also runs an OscillatorNode of that type beside the saw that makes the pulse; still oscillators, still the subject of the bench, nothing sampled.

## reverb/

| File | What | Origin | First used |
|---|---|---|---|
| `vocal.mp3` | 6.4 s lead vocal, dry: a phrase and then three seconds of silence, so the tail is heard on its own | Bars 17 to 19.7 (40.80 to 47.20 s) of the Paper Kites stem generated with Suno on Mike's account for the Balance Desk, August 2026; that bench uses bars 22 to 25 of the same song, so no second of this take is heard twice on the estate | Reverb bench |
| `guitar.mp3` | 8.0 s electric guitar phrase, dry, followed by 2.2 s of silence | 19.4 to 27.4 s (bars 8.3 to 11.8) of the Dry Groove stem generated with Suno on Mike's account for the Automation Lane, August 2026; that bench uses bars 18 to 21 | Reverb bench |
| `snare.mp3` | 1980s drum-machine snare, one shot | The same one-shot as `midi/electronic-snare.mp3` from the Piano Roll's kit, copied here so the folder is complete; it is the sound the gated-reverb idiom was built on | Piano Roll; Reverb bench |

Both phrases were cut dry and measured dry: the vocal is 40 dB down within 0.08 s of the phrase ending and the guitar within 0.28 s, so what the bench adds is the only reverb in the file. Levels for the ear are set by `scripts/measure-reverb.mjs`, not by the files' own RMS (BENCH-STANDARD, 29 Aug 2026). Everything here was generated on Mike's own Suno and ElevenLabs accounts; nothing is a third-party recording with a licence to track. If a file is replaced, update this table in the same commit.

## acoustics/ (no folder: the Acoustics bench reads `reverb/` and makes its own tones)

The Acoustics bench (2026-09-12) adds no audio to the estate. Two of its three stations have no recording to make: the subject of Loudness is a tone and the subject of Masking is a band of noise, which is the one place the Bench Standard admits an oscillator (§3 law 5: "Oscillators may exist only where the subject IS the oscillator"). So the tone is a sine `OscillatorNode` at 100 Hz, 1 kHz, 3 kHz or 10 kHz; the target is a second sine at 1 or 2 kHz; and the masker is a two-second `AudioBuffer` of seeded noise generated in the graph and looped, through a `BiquadFilterNode` band-pass whose Q is set from Zwicker's critical bandwidth at the centre frequency. `synthesis` is declared on the frame; law 9 still fails any bench that does not declare it.

The Room station plays measured stems, and it reads the Reverb bench's folder rather than taking a copy: `reverb/snare.mp3` (one 1980s drum-machine shot, which shows a bounce arriving) and `reverb/vocal.mp3` (a phrase and then silence, which shows the colouration a comb puts on a sustained sound). Both were cut dry and are already credited above. The variety rule of 28 August is met without a new folder because the two stems were cut for exactly this job, and because the alternative was generating audio this bench does not need. Levels for the ear are set by `scripts/measure-acoustics.mjs`, not by the files' own RMS.

## sequence/ (no folder: the Sequence bench reads `midi/` and makes its own voice)

The Sequence bench (2026-09-16) adds no audio to the estate. Its drums are three of the Piano Roll's electronic kit, read from that folder rather than copied: `midi/electronic-kick.mp3`, `midi/electronic-snare.mp3` and `midi/electronic-chat.mp3`, the 1980s drum-machine one-shots generated on Mike's ElevenLabs account on 30 Aug 2026 and credited above. The synth voice is the bench's own, on the Piano Roll's precedent (the sequenced part is a synth, the papers' own part): per note two sawtooth `OscillatorNode`s nine cents apart into a `BiquadFilterNode` low-pass of its own, every filter's frequency and Q driven from one shared `ConstantSourceNode` pair, into a `GainNode` the envelope closes. `synthesis` is declared on the frame; law 9 still fails any bench that does not declare it. Levels for the ear are set by `scripts/measure-seq.mjs`, not by the files' own RMS.
