...
const COMPRESSION_EXPANSIONS = [
  ...
  // --- Dynamics course (Task 6, learn-rollout-wave1): Chapter 1 dynamic-range, new rows ---
  { trigger: "is an automatic fader", content: "Calling a compressor an \u2018automatic fader\u2019 captures the whole idea in one image: instead of a human hand pulling a fader down every time a level gets too loud, a circuit does the same job continuously, tracking the signal and reacting the instant it crosses a chosen point." },
  { trigger: "average (RMS) level", content: "RMS (root-mean-square) is a way of measuring a signal's average level over time, rather than just its instantaneous peaks \u2014 it correlates much more closely with how loud something is perceived to be. Raising the average/RMS level (without necessarily raising the peak level) is exactly what compression followed by makeup gain achieves, which is why a compressed track can sound louder even though its loudest moments haven't grown." },

  // --- Dynamics course: Chapter 2 compressor-controls, new row ---
  { trigger: "the 1:1 diagonal", content: "Below the threshold, a compressor does nothing at all, so a graph of input level against output level is a straight 45-degree line: whatever goes in, at whatever level, comes out unchanged. That line is the 1:1 diagonal \u2014 the starting point every transfer curve is drawn from before the threshold and ratio bend it." },

  // --- Dynamics course: Chapter 3 attack-release, new row ---
  { trigger: "audibly breathes out of step with the track", content: "Pumping happens when the compressor's release time doesn't match the tempo of the music \u2014 the gain reduction is still recovering from the last loud hit when the next one arrives, so the overall level visibly rises and falls out of sync with the beat. Retiming the release to the track's tempo (or using a tempo-synced release) is the standard fix." },

  // --- Dynamics course: Chapter 4 dynamics-family, new rows ---
  { trigger: "a hard ceiling used to stop clipping", content: "A limiter's ceiling is set by its threshold, but unlike a normal compressor's threshold (which only slows the signal down), a limiter's threshold is a wall nothing crosses \u2014 because the ratio is effectively infinite, no amount of extra input level produces any extra output above that point." },
  { trigger: "increases dynamic range rather than reducing it", content: "An expander is a compressor with its logic flipped below the threshold: instead of pulling loud signal down (which narrows the gap between loud and quiet), it pushes quiet signal further down (which widens that gap). That's why an expander is described as increasing dynamic range \u2014 the opposite job to compression." },
  { trigger: "cut by a fixed amount, its range", content: "A gate's range (sometimes called depth) sets how far the signal is pulled down once it's below the threshold \u2014 for example a range of \u221240 dB permanently drops anything under threshold by 40 dB, rather than scaling the cut by a ratio the way a compressor or expander does." },
  { trigger: "the trigger and the target separate", content: "Normally a compressor's detector listens to the exact same signal it's turning down \u2014 trigger and target are the same track. Side-chain processing breaks that link: the detector is fed a completely different signal (the key input), so what decides the gain reduction (the trigger) and what actually gets turned down (the target) become two separate tracks." },
  { trigger: "ducking it out of the kick's way", content: "Ducking describes the audible dip in level that the target track takes every time the trigger fires \u2014 in a kick-ducks-bass setup, the bass visibly (and audibly) drops in level in time with every kick hit, then recovers, making room in the low end for the kick to punch through cleanly." },
  { trigger: "its character set almost entirely by the release time", content: "Two side-chain ducks with identical threshold and ratio settings can sound completely different depending on release time alone: a fast release creates a short, tight rhythmic stab; a slow release creates a longer, more obvious \u2018breathing\u2019 pump that barely recovers before the next trigger hit arrives." },
];

const DELAY_EXPANSIONS = [
  // --- Delay course (Task 9, learn-rollout-wave1): Chapter 2 feedback-types, new row ---
  { trigger: "quieter AND darker together", content: "This is the audible signature of analogue tape feedback: because the delay's playback signal is fed back into the record head and re-recorded every pass, each repeat loses a little level (like any repeated copy) AND a little top end at the same time, since tape itself has a frequency response that rolls off highs. A clean digital delay's repeats only get quieter, from the feedback gain — never darker — which is why ‘darker as well as quieter’ is the reliable identifying clue for tape." },

  // --- Delay course: Chapter 3 timed-delay, new rows ---
  { trigger: "tractable family worth memorising", content: "The reason 60, 100, 120 and 150 BPM appear together as a family is that they all divide 60,000 without a remainder: 1000, 600, 500 and 400 ms respectively. Other common tempos, like 90 BPM, don't divide as cleanly (60,000 ÷ 90 = 666.67 ms) — which is exactly why calculator-free practice questions are built around the values that do, not around every tempo a real song might use." },
  { trigger: "divides cleanly by three", content: "Not every quarter-note value splits into a whole number of milliseconds for a triplet — 600 ms does, because 600 ÷ 3 = 200 exactly, but a value like 500 ms (120 BPM) gives 500 ÷ 3 = 166.67, which isn't calculator-free. Picking a BPM whose quarter-note value is already a multiple of three, such as 100 BPM's 600 ms, is what keeps a triplet calculation mentally tractable." },

  // --- Delay course: Chapter 4 stereo-adt, new row ---
  { trigger: "two delay lines with crossed feedback", content: "This is the structural definition mark schemes credit over a looser description like ‘bounces left and right’: each delay line's feedback loop is wired into the OTHER line's input rather than its own, so left triggers right and right triggers left. A single delay panned to one side, or two delay lines with SEPARATE (uncrossed) feedback loops, would not produce the alternating bounce — the crossing is the essential detail." },
];

const REVERB_EXPANSIONS = [
  // --- Reverb course (Task 4, learn-rollout-wave2): Chapter 3 spring-plate, plate-emt140 row ---
  { trigger: "EMT 140", content: "The EMT 140 is the reference's own named example of a plate reverb unit: a large tensioned steel sheet, driven at the centre, picked up at the edges — dense and bright with no sense of room space, because there is no room geometry for it to imprint on the reflections. Its movable damping plate, a separate sheet that can be brought closer to or further from the vibrating steel, is a purely mechanical decay-time control — the plate-reverb equivalent of a digital RT60 knob." },

  // --- Reverb course: Chapter 4 digital-reverb, algorithmic-plumbing + convolution-fingerprint rows ---
  { trigger: "comb filter", content: "A comb filter recirculates a signal through a short delay, over and over, so the output is the original plus many decreasing copies of itself spaced at equal intervals — plotted on a frequency graph, the boosts and cuts this creates look like the teeth of a comb, which is where the name comes from. In an algorithmic reverb, a bank of comb filters running in parallel is what generates the initial decaying train of echoes." },
  // beyond reference text — accurate, flagged for Mike's review (see w2-task-4-report)
  { trigger: "allpass filter", content: "An allpass filter passes every frequency through at equal level — nothing is boosted or cut — but it delays different frequencies by different amounts, scrambling their phase relationships without colouring the tone. Feeding a comb filter's echo train through allpass filters is what smears those still-countable echoes into one dense, smooth tail, rather than a row of separate audible repeats." },
  { trigger: "impulse response", content: "An impulse response is the recorded proof of what a real space (or a piece of hardware) does to a single test click: fire the impulse, capture every reflection that returns, and the result is a fingerprint of that space's early reflections and decay, rolled into one file. A convolution reverb applies that fingerprint to any input signal — realistic, because it really did happen in that room, but fixed, because the recording cannot be asked for a different decay time." },

  // --- Reverb course: Chapter 2 the-tail, absorption-damping row ---
  // beyond reference text — accurate, flagged for Mike's review (see w2-task-4-report)
  { trigger: "standing waves", content: "A standing wave forms when a sound wave reflects back on itself between two parallel surfaces, often opposite walls, reinforcing at some points while cancelling at others — piling up energy, usually bass, at room boundaries and corners rather than spreading it evenly. Corner bass traps are absorptive treatment placed exactly where standing waves pile up, which is why they specifically target the low end rather than the whole spectrum." },
  { trigger: "highs die first", content: "RT60 is usually quoted as a single number, but a room's decay is not the same length at every frequency: high frequencies lose energy to air absorption and soft surfaces faster than low frequencies do, so a room's true RT60 is shorter at the top of the spectrum and longer at the bottom. 'Highs die first' is the shorthand for this, and treating RT60 as one flat number across all frequencies is a commonly flagged simplification." },
];

const SAMPLING_EXPANSIONS = [
  // --- Sampling course (Task 10, learn-rollout-wave2): Chapter 1 the-sampler, why-sample-drums row ---
  { trigger: "needs clearance before release", content: "Clearance is a legal step, not a technical one: sampling any fragment of a commercial recording needs permission from the owners of both the master recording and the underlying song before the finished track can be released. No compulsory licence exists for sound recordings, so that permission cannot be assumed or bought automatically — it has to be sought and granted on a case-by-case basis for every sample used." },

  // --- Sampling course: Chapter 2 rate-depth, bit-depth-staircase row ---
  { trigger: "a recording's file size", content: "The arithmetic is genuinely simple, just not mental-maths simple: data per second equals sample rate times bit depth times number of channels, divided by eight to convert bits into bytes. Worked through for CD-quality stereo — 44,100 times 16 times 2, divided by 8 — that comes to 176,400 bytes every second, which is why CD-quality stereo audio runs to roughly 10 MB a minute, the figure the spec actually quotes." },

  // --- Sampling course: Chapter 3 the-edit, clicks-zero-crossings row ---
  { trigger: "reading hex numbers", content: "Before waveform displays existed on screen, finding a clean loop point meant reading a stream of raw numbers by hand: Digidesign's own founders are the reference's example, spending hours reading hex numbers to locate the zero crossings needed for a usable loop. Modern sample editors automate that search instantly and show the crossing visually, but the underlying test they are running is exactly the same one — is the waveform's value at this exact sample point zero or not." },

  // --- Sampling course: Chapter 4 the-map, multisampling-zones row ---
  { trigger: "wider list of playback settings", content: "When an exam question asks what needs setting once a sample is assigned to a sampler, it wants this specific list: volume, key and zone, root note, start and end points, one-shot or loop, direction, and pan. Envelope, filter and LFO settings are explicitly not credited here, because those are synthesis controls applied after the mapping is already done, not part of the mapping itself — and sequencing settings like tempo or BPM are refused for the same reason." },
  { trigger: "velocity layering", content: "Precision matters here more than almost anywhere else in the topic: the credited phrase is velocity layering — recordings assigned to soft and hard playing, switched by how firmly a key is struck — and the mark scheme specifically refuses the bare word velocity on its own, with nothing attached. The distinction is not pedantic: 'velocity' alone could describe almost any MIDI parameter, while 'velocity layering' names the specific multisampling technique being tested." },

  // --- Sampling course: Chapter 5 transforms, repitch-stretch-shift + chop-layer-culture rows ---
  { trigger: "warp mode", content: "Warp modes are a DAW's menu of algorithms for time-stretching and pitch-shifting, each tuned to a different kind of material — Beats mode for drums, Complex mode for mixed or tonal material are the two the reference names. Picking the wrong one for the job, a drum-tuned mode applied to a held vocal note or a tonal mode applied to a busy break, is exactly the kind of audible error the exam credits you for identifying by ear." },
  { trigger: "Amen break", content: "The Winstons' four-bar drum break, the 'Amen' bar, is the reference's own worked example of what chopping and layering can do: looped whole as a foundation, chopped into individual hits and resequenced into patterns its own drummer never played, sped from its original 138 BPM into jungle or slowed down into hip-hop. Bronx DJs had already extended breaks like this with two copies of the same record, years before samplers existed — the samplers of the late eighties absorbed a technique the turntable got to first." },
];

const DISTORTION_EXPANSIONS = [
  // --- Distortion course (Task 3, learn-rollout-wave3): drive chapter, what-distortion-is row ---
  { trigger: "gentle to extreme", content: "The spec's own four-type ladder mirrors what the lab's four modelled circuits show in miniature: soft, gradual clipping — associated with overdrive pedals and analogue tape — sits towards the gentle end, while a heavily driven, near-square clip (fuzz) sits at the extreme end. Saturation, the gentlest of all, is the everyday warmth of analogue gear driven just past its comfort zone, rather than any dramatic reshaping of the wave." },

  // --- Distortion course: drive chapter, drive-tone-level row ---
  { trigger: "for a parallel effect", content: "Blending a fully distorted signal back in parallel with the clean original, rather than replacing it outright, is a common production move: it keeps the punch and clarity of the dry source while layering in the harmonic weight and perceived loudness the distortion adds, without losing the original's definition entirely." },

  // --- Distortion course: drive chapter, harmonic-colour row ---
  { trigger: "tube saturation", content: "A single-ended valve stage doesn't treat a signal's rise and fall symmetrically the way a solid-state clipper does — the positive half of the wave compresses differently from the negative half, and that lopsidedness is exactly what generates even-order harmonics. It's the same asymmetry principle behind why tube gear is so consistently described as 'warm': even harmonics (2nd, 4th, 6th) sit in tune with the fundamental in a way the ear reads as musical rather than harsh." },

  // --- Distortion course: drive chapter, analogue-vs-digital-dirt row ---
  { trigger: "quantisation as an effect", content: "Quantisation is the same rounding-to-levels process the Sampling course covers for a recording's bit depth — fewer bits means a coarser staircase of amplitude levels and more audible quantisation noise. Bitcrushing takes that same staircase and uses it on purpose, as a deliberate, gritty effect rather than something to avoid." },
];

const MIDI_EXPANSIONS = [
  // --- MIDI course (Task 4, learn-rollout-wave3): notes-as-data chapter, what-midi-carries row ---
  // beyond named sources — accurate, flagged for Mike's review (see w3-task-4-report)
  { trigger: "a few kilobytes rather than megabytes", content: "Audio recording captures the actual sound pressure many thousands of times a second — real sample values, covered in the Sampling course. MIDI captures none of that: a Note On message is only a status byte plus two small data bytes, so a whole performance becomes a short list of numbers instead of a continuous waveform. That is also why every note stays editable after the fact — change one number (the pitch, the velocity, the timing) and the instruction itself changes, with nothing to re-record." },

  // --- MIDI course: notes-as-data chapter, two-ways-in row ---
  // beyond named sources — accurate, flagged for Mike's review (see w3-task-4-report)
  { trigger: "too fast, too complex, or too precise to perform live", content: "A rapid run of very short notes, a drum pattern quantised to machine precision, or a chord voicing with more notes than fingers available are typical reasons a producer reaches for step input instead of playing a part live. The trade runs the other way too: step-entered parts can sound mechanically perfect, which is exactly why producers often choose real-time input on purpose — not because it's easier, but because they want the imperfect timing a live performance carries." },

  // --- MIDI course: notes-as-data chapter, quantise-and-edit row ---
  // beyond named sources — accurate, flagged for Mike's review (see w3-task-4-report)
  { trigger: "swing or percentage strength setting", content: "Quantise strength is a percentage dial between a note's original position and the grid: 100% is a hard quantise, pulling every note fully onto the nearest grid line; 50% moves each note only halfway there, keeping half of its original timing offset. Swing works on the same principle but only touches the off-beat subdivisions — nudging every other note later by a percentage so a straight rhythm gains a shuffled, groove feel instead of sitting dead-on the grid." },

  // --- MIDI course: notes-as-data chapter, beyond-the-note row ---
  { trigger: "LSB and MSB", content: "Splitting a 14-bit number into two 7-bit bytes is arithmetic: the least significant byte (LSB) is the value's remainder when divided by 128, and the most significant byte (MSB) is how many whole 128s fit into it. At the centre position, 8192, that works out to MSB 64 and LSB 0 — 64 × 128 + 0 = 8192. The receiving instrument reverses the sum to reconstruct the original 14-bit value." },
];

const RECORDING_EXPANSIONS = [
  // --- Recording course (Task 5, learn-rollout-wave3): signal-path chapter, the-chain row ---
  // beyond named sources — standard, uncontested extension of the specSummary
  // vocabulary skeleton; flagged for Mike's review (see w3-task-5-report)
  { trigger: "analogue becomes digital", content: "Turning analogue into digital means measuring the electrical signal's voltage at a fixed rate (the sample rate) and rounding each measurement to the nearest available step (set by the bit depth) — the next row covers exactly those two settings. Nothing before the converter in the chain is ever measured or rounded at all; it is still a continuously varying electrical signal." },

  // --- Recording course: signal-path chapter, captured-quality row ---
  // cross-link named by topic only, per the self-containment law — the
  // Digital & Analogue course has not shipped yet
  { trigger: "quantisation noise", content: "Quantisation is the rounding step itself — every measurement gets snapped to the nearest available step, and how many steps are available is set by the bit depth. The full mechanics of how a continuous analogue voltage becomes a stream of digital numbers belong to the Digital & Analogue lesson; this course only needs the trade-off it creates here — more bits means finer steps and less rounding error, but a larger file." },

  // --- Recording course: signal-path chapter, hearing-yourself row ---
  // beyond named sources — standard, uncontested extension of the specSummary
  // vocabulary skeleton; flagged for Mike's review (see w3-task-5-report)
  { trigger: "sidesteps the buffer entirely", content: "Direct monitoring works because the audio interface itself can route its own input straight back out to its own output, in hardware, before that signal is ever sent up to the computer and into the DAW's buffer at all. That is why it adds effectively no latency — but it also means you only hear what the interface itself can add (sometimes a built-in reverb), not any DAW-based plugins running on that track." },
];

const DIGITAL_ANALOGUE_EXPANSIONS = [
  // --- Digital & Analogue course (Task 6, learn-rollout-wave3): conversion chapter, two-worlds row ---
  { trigger: "hiss, wow and flutter", content: "These are the classic audible fingerprints of analogue tape: hiss is a constant high-frequency noise floor from the tape medium itself, wow is a slow pitch wobble from speed variation, flutter is the same wobble at a faster rate. A digital copy carries none of these, because copying numbers exactly reproduces the same numbers — there is no physical medium to wear down between generations." },

  // --- Digital & Analogue course: conversion chapter, the-round-trip row ---
  // beyond named sources — standard, uncontested extension of the ADC/DAC
  // definitions; flagged for Mike's review (see w3-task-6-report)
  { trigger: "reverses that process", content: "A DAW's stored numbers are silent on their own — a loudspeaker cone moves in response to a continuously varying voltage, not a list of numbers. The DAC's job is to rebuild that voltage from the discrete measurements the ADC took, turning the stream of numbers back into something a speaker can actually follow. Skip this stage and there is nothing left for the monitors to reproduce." },

  // --- Digital & Analogue course: conversion chapter, fast-enough row ---
  // cross-link named by course + chapter title, per the self-containment law
  { trigger: "temporal resolution", content: "Temporal resolution is the time-axis half of a recording's quality — how finely spaced each measurement is — separate from bit depth's amplitude-axis resolution covered next. The Sampling course's From Sound to Numbers chapter works through the full sample-rate-and-Nyquist arithmetic in depth, including the file-size maths this chapter doesn't need; here, the pipeline only needs what sample rate buys and what breaking its rule costs." },

  // --- Digital & Analogue course: conversion chapter, deep-enough row ---
  { trigger: "noise floor", content: "The arithmetic behind the noise floor is a simple formula, not a memorised number: quantisation levels = 2^n, where n is the bit depth — 16 bits gives 2^16 = 65,536 levels, 24 bits gives 2^24, nearly 16.8 million. More levels means finer rounding and a lower noise floor; fewer levels means a coarser rounding and a floor high enough to be heard as grit under quiet passages." },
];

const NUMERACY_EXPANSIONS = [
  // --- Numeracy course (Task 7, learn-rollout-wave3): the-numbers chapter, pitch-numbers row ---
  { trigger: "1 kHz ⇄ 1 ms", content: "This shortcut generalises because T = 1/f: double the frequency and the period halves, halve the frequency and the period doubles — the same doubling relationship an octave uses. 2 kHz ⇄ 0.5 ms, 500 Hz ⇄ 2 ms — once the 1 kHz ⇄ 1 ms anchor is memorised, the rest of the family follows from octave doubling and halving, not fresh division." },

  // --- Numeracy course: the-numbers chapter, level-numbers row ---
  // TASK-7 REVIEW FIX (2026-07-17): trigger and content rewritten to track
  // the row's new programme-sense wording (dynamic range = dB gap between a
  // signal's quietest and loudest parts, not ceiling − noise floor — see
  // numeracy.js's row 2 comment for the full rationale). The −50 dBFS /
  // −10 dBFS verse-chorus figures are invented illustrative values, not
  // sourced from a named file — flagged in the task report.
  { trigger: "a verse at −50 dBFS and a chorus at −10 dBFS give 40 dB of range", content: "These are illustrative examples, not fixed exam values — real quiet and loud passages vary by track. What is fixed is the method: dynamic range = loudest part − quietest part, always a subtraction in dB (−10 − (−50) = 40). The Digital & Analogue course's Crossing the Line chapter reaches a dynamic-range figure a different way — from bit depth, roughly 6 dB per bit — a related number worked out from a different starting point, not a duplicate of this one." },

  // --- Numeracy course: the-numbers chapter, tempo-numbers row ---
  // cross-link named by course + chapter title, per the self-containment law
  { trigger: "divide, don't guess", content: "This is the same formula the Delay course's Timed Delay & the Maths chapter builds a full worked family from — 60, 100, 120 and 150 BPM anchors, with dotted-eighth and triplet-quarter workings shown step by step. Numeracy only needs the shortcut here: divide 60,000 by the BPM, then apply ×1.5 for dotted or ×⅔ for triplet — that fuller chapter is where the complete derivation and reference table live." },

  // --- Numeracy course: the-numbers chapter, size-numbers row ---
  { trigger: "sample rate × bit depth × channels × time", content: "Worked from scratch, not just ratios: a 60-second mono recording at 44.1 kHz, 16-bit needs 44,100 × 16 × 1 × 60 ÷ 8 bytes. Step by step: 44,100 × 16 = 705,600; × 1 (mono) stays 705,600; × 60 seconds = 42,336,000; ÷ 8 (bits to bytes) = 5,292,000 bytes = 5.292 MB. This is exactly the heavy arithmetic the ratio shortcuts above are built to avoid." },
];

const LEADS_AND_SIGNALS_EXPANSIONS = [
  // --- Leads & Signals course (Task 8, learn-rollout-wave3): connections chapter, the-connectors row ---
  // sourced from AudioLeadsFlashcards.jsx card i1's own practicalExample
  { trigger: "locks in place", content: "XLR's twist-lock shell is what earns it the reliability professional touring and broadcast rigs depend on: once connected, the two halves lock together and cannot be pulled apart by an accidental tug on the cable — a plain 1/4\" jack or an RCA plug has no equivalent mechanism, so either can work loose mid-performance in a way an XLR connection cannot." },

  // --- Leads & Signals course: connections chapter, balanced-vs-unbalanced row ---
  // beyond named sources — standard, uncontested extension of AudioLeadsFlashcards.jsx
  // card b3's "cancel out interference" claim; flagged for Mike's review (see w3-task-8-report)
  { trigger: "flipped and summed", content: "The maths behind the flip-and-sum trick: noise couples onto a cable from the outside, so it arrives as an almost-identical extra signal on both the hot and cold conductors. Cold already carries an inverted copy of the wanted signal, so flipping cold's polarity back the right way up at the receiver and adding it to hot puts the two wanted-signal copies back in phase (they add) while the two noise copies — now opposite in phase to each other — cancel out." },

  // --- Leads & Signals course: connections chapter, levels-and-di row ---
  // source-wins editorial note — see this file's own header comment on Row 3
  // for the full disclosure of why a2's confusing "high Z / low Z" framing is not used
  { trigger: "DI box bridges the gap", content: "A passive DI box does its job with a transformer, not any active electronics: it steps a guitar or bass's high-impedance, unbalanced signal down to a low-impedance, balanced signal at roughly mic level — exactly what a mixing desk's mic preamp input (and its XLR socket) is built to receive. That balanced, low-impedance signal is also far more resistant to noise over a long cable run than the original high-impedance instrument signal would have been." },

  // --- Leads & Signals course: connections chapter, order-and-routing row ---
  // cross-link named by course + chapter title, per the self-containment law
  { trigger: "a send taps a copy to a shared effect", content: "This one-line distinction is as deep as this course goes — the Reverb course's Wiring It In: Sends, Inserts & Faders chapter builds the full treatment from here: why routing reverb as an insert instead of a send makes it pan with its channel (a recurring examiner complaint), the separate pre-fader/post-fader tap-point decision, and the automated fade trick that depends on getting both right. Read that chapter for the deeper working; this course only needs the chain order and the basic insert-vs-send split." },
];

// Combine all expansions into a single searchable list
export const ALL_EXPANSIONS = [...SYNTHESIS_EXPANSIONS, ...EQ_EXPANSIONS, ...COMPRESSION_EXPANSIONS, ...DELAY_EXPANSIONS, ...REVERB_EXPANSIONS, ...SAMPLING_EXPANSIONS, ...DISTORTION_EXPANSIONS, ...MIDI_EXPANSIONS, ...RECORDING_EXPANSIONS, ...DIGITAL_ANALOGUE_EXPANSIONS, ...NUMERACY_EXPANSIONS, ...LEADS_AND_SIGNALS_EXPANSIONS];
