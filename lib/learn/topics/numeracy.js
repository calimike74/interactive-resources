...
// and "0 dBFS" as the digital ceiling is lm-q6's own explanation, all from
// LevelsMeteringAssessment.jsx. The −60 dBFS noise-floor figure used for the
// row's dynamic-range worked example is NOT sourced from any named file —
// it is an invented illustrative value, chosen only because 0 − (−60) = 60
// is a clean non-calculator subtraction. Flagged prominently in the task
// report. This row deliberately does NOT reuse dynamics.js's own −40 dBFS /
// −6 dBFS / 34 dB whisper-shout illustration (visible in the reused
// dynamic-range-gap diagram's fixed canvas numbers, carried over unmodified
// from that course) — dynamics.js is not a source named in this task's
// brief, so its specific figures are not mined here even though the reused
// diagram still displays them; the row's own prose uses its own,
// independently-invented illustrative numbers instead. See the task report.
//
...
export const NUMERACY_CHAPTERS = [
    {
        ...
        rows: [
            ...
            },
            {
                ...
                description: "The dB scale is logarithmic, not linear: equal steps are equal ratios, not equal jumps. +6 dB doubles voltage; −6 dB halves it — just doubling or halving, no calculator needed. A digital system's ceiling is 0 dBFS, the loudest sample it can hold before clipping. Dynamic range is the gap between that ceiling and the noise floor: a floor at −60 dBFS gives 60 dB of range.",
