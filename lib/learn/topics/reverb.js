...
// (<=~70 words), animation, optional audio/interactive, assessment.
//
// Structural note on the three "audio pair" rows (pre-delay-gap,
// distance-rd-ratio, rt60-decay): the wave-2 chapter map's placement
// decisions call for an A/B audio comparison (e.g. verb-dry <-> verb-hall).
// The existing row.audio shape (components/learn/LearnSpineLayout.js:253,
// components/learn/AudioBlock.js) renders exactly one preset per row. To
// avoid shipping a broken control, each of these three rows carries the
// single most illustrative preset as `audio.preset` (renders correctly
// today, no LearnSpineLayout change needed) plus `comparePreset`/
// `compareLabel` fields holding the second half of the pair, dormant until
// LearnSpineLayout is extended to render both. See w2-task-4-report.md
// "Concerns" for the full note.
...
export const REVERB_CHAPTERS = [
    {
        ...
        rows: [
            ...
            },
            {
                ...
                audio: {
                    preset: 'verb-predelay',
                    label: 'Hold to hear the hall with an 80 ms gap before the reverb begins — pre-delay, not an echo.',
                    comparePreset: 'verb-hall',
                    compareLabel: 'Compare: the same hall with no pre-delay gap — the reverb blooms immediately after the direct sound.',
                },
                assessment: {
                    id: 'pre-delay-gap',
            ...
            },
            {
                ...
                audio: {
                    preset: 'verb-hall',
                    label: 'Hold to hear the tick mostly through the room — a high R/D ratio, source pushed into the distance.',
                    comparePreset: 'verb-dry',
                    compareLabel: 'Compare: the same tick almost entirely dry — a low R/D ratio, source close and present.',
                },
                assessment: {
                    id: 'distance-rd-ratio',
        ...
        rows: [
            {
                ...
                audio: {
                    preset: 'verb-hall',
                    label: 'Hold to hear the 1.8 s hall tail — a long RT60, still falling well after the tick.',
                    comparePreset: 'verb-room',
                    compareLabel: 'Compare: the 0.4 s room tail — a short RT60, the decay already gone.',
                },
                assessment: {
                    id: 'rt60-decay',
