...
// Not yet wired into lib/learn/topics/index.js — that switch is Task 5's job.
// Legacy rows below (what-eq-solves, graphic-eq, frequency-bands, parametric-eq,
// q-factor, routing, comparison) are copied verbatim from EQ_TOPIC above: same
// heading/description/animation/assessment. New rows are marked in comments.
...
export const EQ_CHAPTERS = [
        ...
        rows: [
            {
                ...
                description: 'Every sound contains a mix of frequencies. Some are too loud, others too quiet. EQ lets you boost or cut specific frequency ranges to shape the tonal balance of a signal — fixing problems or enhancing character.',
                animation: 'frequency-spectrum',
                audio: { preset: 'eq-tone-flat', label: 'Hold to hear the flat, unprocessed baseline tone — nothing boosted, nothing cut, nothing shaped yet.' },
                assessment: {
                    id: 'what-eq-solves',
                    question: 'A recording of an acoustic guitar sounds thin and lacks warmth. Which approach would be most appropriate?',
                    options: [
            ...
            // --- new row ---
            {
                ...
                audio: { preset: 'eq-presence-boost', label: 'Hold to hear a presence boost push 3 kHz forward — brighter and more edgy, a creative EQ move.' },
                assessment: {
                    id: 'boost-vs-cut',
    ...
    },
    {
        ...
        examAnchor: {
            ...
            examTip: '2025 A Q4(c) credited this exact HPF chain: cuts low frequencies, cut-off in the 20–160 Hz range, reduces rumble, hum, plosives and proximity effect, makes space in the mix — without affecting vocal tone. Naming the mechanism alongside the number is what separates full marks from partial credit.',
        },
        ...
        rows: [
            // --- new row ---
            {
                ...
                audio: { preset: 'eq-highpass', label: 'Hold to hear a high-pass filter strip away everything below 300 Hz, leaving only the top end.' },
                assessment: {
                    id: 'high-pass-low-pass',
            ...
            // --- new row ---
            {
                ...
                description: 'Below the cut-off, a filter\'s steepness is measured in dB per octave — how much quieter the signal gets for every doubling, or halving, of frequency. Each filter pole contributes 6 dB per octave, so 12 dB per octave is a standard, gentle slope; 24 dB per octave is steep; 48 dB per octave is close to a brick wall. A steeper slope removes unwanted frequencies faster, but too steep can sound unnatural.',
            ...
            // --- new row ---
            {
                ...
                audio: { preset: 'eq-low-shelf-boost', label: 'Hold to hear a low-shelf boost pile weight below 200 Hz — warmer and thicker, edging toward boomy.' },
                assessment: {
                    id: 'shelving-filters',
    ...
    },
    {
        ...
        rows: [
            ...
            // --- new row ---
            {
                ...
                description: 'Every EQ decision happens in the context of the whole mix, not in solo. A part that sounds thin on its own might already have room reserved by another instrument, and cutting the wrong track to fix it just creates a new problem. Matching tasks — making one section\'s tone match another — mark you down for reaching for volume instead of EQ, or for changing audio outside the stated bars.',
