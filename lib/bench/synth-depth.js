...
import {
    WAVES, OSC2, FILTERS, LFO_TARGETS, LFO_SHAPES, VOICES, GLIDES, PARTS, SECTIONS, SECTION_IDS, TASKS, GRADE_WORD, BPM,
...
export const DEPTH_LINES = {
    core: 'the bench names what you hear and tells you what to try. The stage is the wave leaving the synth beside its harmonics, the filter drawn over them in gold: drag its dot and the Cutoff dial follows. The keys at the foot play the voice, and so do A to K.',
...
export function hearingLine(state) {
    ...
    const osc = state.osc2 === 'pair'
        ? `two ${WAVES[state.wave].label.toLowerCase()} waves ${state.detune > 0 ? `${state.detune} cents apart` : 'in unison'}`
        : state.osc2 === 'sub' ? `${WAVES[state.wave].said} with a square sub an octave down` : WAVES[state.wave].said;
    ...
    const env = state.attack >= 200 ? `Each note swells in over ${fmtMs(state.attack)}` : `Each note starts in ${fmtMs(state.attack)}`;
    const rel = state.release >= 800 ? `and rings for ${fmtMs(state.release)} after the key lifts` : `and stops ${fmtMs(state.release)} after the key lifts`;
    const lfo = lfoOn(state) ? ` The LFO adds ${lfoWord(state)} at ${fmtRate(state.lfoRate)}.` : '';
...
const PRESET_MOVES = {
    ...
    judgeBass: 'switch to A-level and touch the Envelope: a 600 ms attack on a bass is the fault, and the sections say so',
    judgePad: 'switch to A-level and press Poly in the More row: the chord\'s four notes come back',
};
export function nextMove(state) {
    if (state.presetId && PRESET_MOVES[state.presetId]) return PRESET_MOVES[state.presetId];
    if (state.wave === 'sine') return 'switch the wave to Saw and watch the harmonics appear for the filter to work on';
...
const SECTION_OF_LAST = { osc: 'osc', wave: 'osc', octave: 'osc', detune: 'osc', osc2: 'osc', filter: 'filter', cutoff: 'filter', res: 'filter', envAmt: 'filter', stage: 'filter', env: 'env', attack: 'env', decay: 'env', sustain: 'env', release: 'env', lfo: 'lfo', lfoTarget: 'lfo', lfoRate: 'lfo', lfoDepth: 'lfo', lfoShape: 'lfo', voices: 'voices', glide: 'voices', level: 'voices' };
export const sectionOfLast = (last) => SECTION_OF_LAST[last] || null;

function settingOf(state, section) {
    const s = state;
    if (section === 'osc') {
        const o = s.osc2 === 'pair' ? `two ${WAVES[s.wave].label.toLowerCase()} waves, ${s.detune} cents apart` : s.osc2 === 'sub' ? `${WAVES[s.wave].said} with a square sub-oscillator an octave down` : WAVES[s.wave].said;
        return `${o}, ${octaveSaid(s.octave)}: ${WAVES[s.wave].harmonics}, ${WAVES[s.wave].character}`;
    }
    ...
    return `${VOICES[s.voices].said}, ${GLIDES[s.glide].said}`;
}
...
export function judge({ state, last }) {
    ...
    if (!section) {
        ...
        return [
            ...
            seg(4, worst ? `${cap(SECTIONS[worst].name)} first: ${all[worst].why}. The 2024 report credited answers written section by section, under subheadings.` : `Every section suits ${job}. The 2024 report credited answers written section by section, under subheadings.`),
        ];
    }
    const g = all[section];
...
function betterSetting(s, section, grade) {
    ...
    if (section === 'osc') {
        if (part === 'bass') return s.octave > 0 ? 'Press 0 or the octave below.' : 'Choose Square or Saw, and add the Sub in the More row.';
        if (part === 'pad') return 'Choose Pair in the More row and turn Detune to 8 to 15 cents.';
        if (part === 'keys') return 'Choose Saw, in a pair with a slight detune.';
        return 'Choose Square or Saw.';
    }
    if (section === 'filter') {
...
export function open({ state, last }) {
    ...
    const voiceLine = s.voices === 'mono' ? `Mono: each note cuts the last, ${s.glide !== 'off' ? `and the pitch slides to it over ${GLIDES[s.glide].ms} ms, which is portamento` : 'with no slide between them'}.` : 'Poly: each note has its own envelope and filter, so a chord is several of these timelines at once.';
    ...
    return `${envLine} ${filterLine} ${lfoLine}`;
}
