...
import {
    WAVES, OSC2, FILTERS, LFO_TARGETS, LFO_SHAPES, VOICES, GLIDES, ARPS, PARTS, SECTIONS, SECTION_IDS, TASKS, GRADE_WORD, BPM,
...
export function hearingLine(state) {
    ...
    const osc = state.osc2 === 'pair'
        ...
        : state.osc2 === 'sub' ? `${WAVES[state.wave].said} with a square sub an octave down` : state.osc2 === 'fifth' ? `two ${WAVES[state.wave].label.toLowerCase()} waves a fifth apart` : WAVES[state.wave].said;
...
const SECTION_OF_LAST = { osc: 'osc', wave: 'osc', octave: 'osc', detune: 'osc', osc2: 'osc', filter: 'filter', cutoff: 'filter', res: 'filter', envAmt: 'filter', stage: 'filter', env: 'env', attack: 'env', decay: 'env', sustain: 'env', release: 'env', lfo: 'lfo', lfoTarget: 'lfo', lfoRate: 'lfo', lfoDepth: 'lfo', lfoShape: 'lfo', voices: 'voices', glide: 'voices', arp: 'voices', level: 'voices' };
export const sectionOfLast = (last) => SECTION_OF_LAST[last] || null;

function settingOf(state, section) {
    const s = state;
    if (section === 'osc') {
        const o = s.osc2 === 'pair' ? `two ${WAVES[s.wave].label.toLowerCase()} waves, ${s.detune} cents apart` : s.osc2 === 'sub' ? `${WAVES[s.wave].said} with a square sub-oscillator an octave down` : s.osc2 === 'fifth' ? `two ${WAVES[s.wave].label.toLowerCase()} waves a fifth apart (coarse tuning)` : WAVES[s.wave].said;
    ...
    return `${VOICES[s.voices].said}, ${GLIDES[s.glide].said}${s.arp === 'up' ? `, ${ARPS.up.said}` : ''}`;
}
