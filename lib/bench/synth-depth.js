...
export function oscSaid(state) {
    ...
    if (s.osc2 === 'pair') {
        ...
        return single && (w === 'square' || w === 'saw') ? `two ${w} waves ${apart}` : `${base}, doubled ${apart}`;
    }
    if (s.osc2 === 'fifth') return single && (w === 'square' || w === 'saw') ? `two ${w} waves a fifth apart` : `${base}, with a second VCO a fifth above`;
    return base;
}
...
const SECTION_OF_LAST = { osc: 'osc', pulse: 'osc', saw: 'osc', sub: 'osc', noise: 'osc', width: 'osc', pwm: 'osc', subOct: 'osc', octave: 'osc', detune: 'osc', osc2: 'osc', filter: 'filter', cutoff: 'filter', res: 'filter', envAmt: 'filter', stage: 'filter', env: 'env', attack: 'env', decay: 'env', sustain: 'env', release: 'env', lfo: 'lfo', lfoTarget: 'lfo', lfoRate: 'lfo', lfoDepth: 'lfo', lfoShape: 'lfo', voices: 'voices', vca: 'voices', glide: 'voices', arp: 'voices', level: 'voices' };
