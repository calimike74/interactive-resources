...
export function open({ state, last }) {
    ...
    if (last === 'write') {
        return `Touch: the dial wrote a point every 16th of a beat while you held it, replacing what was there, and handed the lane back when you let go. Latch keeps writing the last value to the end of the pass; Write erases as it passes; Read records nothing. ${stored}`;
    }
    if (last === 'shape') {
