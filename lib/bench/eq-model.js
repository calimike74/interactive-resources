...
export const BANDS = {
    hpf: { label: 'HPF', name: 'high-pass filter', type: 'highpass', hzMin: 20, hzMax: 1000, slopes: [12, 24], colour: 'var(--gen-2)' },
    ...
    lpf: { label: 'LPF', name: 'low-pass filter', type: 'lowpass', hzMin: 1000, hzMax: 20000, slopes: [12, 24], colour: 'var(--gen-3)' },
};
...
// and the BiquadFilterNode both take. A 24 dB/oct filter is two sections.
...
export function slopeFacts(id, state) {
    ...
    return { hz, atCutoffDb: r[0].db, octaveDb: r[1].db, slope: b.slope, order: b.slope === 24 ? 4 : 2 };
}
...
export const fmtDb = (db) => `${db > 0 ? '+' : db < 0 ? '−' : ''}${Math.abs(db).toFixed(1).replace(/\.0$/, '')} dB`;
