...
// The bands of a channel-strip EQ, in the order the signal meets them.
// The exam's words: high-pass and low-pass filters (cutoff, slope), low
// and high shelves (gain), and the parametric band (frequency, gain, Q),
// of which the bench offers up to three (28 Aug 2026: a boost in one
// place and a cut in another is the move a real EQ is for). The second
// and third bells are hidden until the student asks for them.
export const BAND_IDS = ['hpf', 'low', 'mid', 'mid2', 'mid3', 'high', 'lpf'];
export const BELL_IDS = ['mid', 'mid2', 'mid3'];
export const BELLS_MAX = 3;
export const BANDS = {
    ...
    mid2: { label: 'Mid 2', name: 'second parametric band', type: 'peaking', hzMin: 100, hzMax: 8000, colour: 'var(--gen-6)' },
    mid3: { label: 'Mid 3', name: 'third parametric band', type: 'peaking', hzMin: 100, hzMax: 8000, colour: 'var(--gen-7)' },
...
export const hasQ = (id) => BELL_IDS.includes(id);
export const hasGain = (id) => id === 'low' || id === 'high' || hasQ(id);
export const hasSlope = (id) => id === 'hpf' || id === 'lpf';
// The bands the console shows: the hidden bells stay out of the way.
export const bellsShown = (state) => Math.max(1, Math.min(BELLS_MAX, state.bells || 1));
export const visibleBands = (state) => BAND_IDS.filter((id) => id === 'mid' || !hasQ(id) || BELL_IDS.indexOf(id) < bellsShown(state));
// The first bell is "Mid" on its own and "Mid 1" once it has company.
export const bandLabel = (state, id) => (id === 'mid' && bellsShown(state) > 1 ? 'Mid 1' : BANDS[id].label);
...
export const DEFAULT_STATE = Object.freeze({
    ...
    mid2: { on: false, hz: 3000, gain: 0, q: 1 },
    mid3: { on: false, hz: 250, gain: 0, q: 1 },
    high: { on: false, hz: 8000, gain: 0 },
    lpf: { on: false, hz: 12000, slope: 12 },
});
const BELLS_AWAY = Object.freeze({ bells: 1, mid2: DEFAULT_STATE.mid2, mid3: DEFAULT_STATE.mid3 });

// How many bells the student wants: the ones revealed come in at once (a
// bell at zero changes nothing, and a dot on the line is the answer to
// "where did it go"), the ones put away go out but keep their settings.
// The dials go to the newest bell.
export function setBells(state, n) {
    const bells = Math.max(1, Math.min(BELLS_MAX, Math.round(n) || 1));
    const was = bellsShown(state);
    const next = { ...state, bells, presetId: null };
    BELL_IDS.forEach((id, i) => {
        if (i === 0) return;
        const shown = i < bells;
        next[id] = { ...state[id], on: shown };
    });
    if (bells > was) next.band = BELL_IDS[bells - 1];
    else if (hasQ(state.band) && BELL_IDS.indexOf(state.band) >= bells) next.band = 'mid';
    return next;
}
...
export const PRESETS = [
    ...
    // A cut where the voice is muddy and a boost where it is missing: the
    // pair of moves a parametric EQ exists for, one bell each.
    {
        id: 'cutboost',
        name: 'Cut and boost',
        blurb: 'Two bells on the voice: a 4 dB cut in the mud and a 3 dB boost in the presence. The pair every vocal EQ answer is built on.',
        state: { graphic: false, source: 'vocal', band: 'mid2', bells: 2, hpf: { on: true, hz: 100, slope: 12 }, low: { on: false, hz: 100, gain: 0 }, mid: { on: true, hz: 350, gain: -4, q: 1.5 }, mid2: { on: true, hz: 3000, gain: 3, q: 1 }, high: { on: false, hz: 8000, gain: 0 }, lpf: { on: false, hz: 12000, slope: 12 } },
    },
    {
        id: 'drums',
        name: 'Drum weight',
...
export function applyPreset(state, presetId) {
    ...
    // a preset puts the extra bells away unless it asks for them
    return { ...state, ...BELLS_AWAY, ...preset.state, presetId };
}
