...
import { BANDS, BAND_IDS, hasGain, hasQ, hasSlope, bandwidthOctaves, slopeFacts, peakOf, regionOf, snapOctave, fmtHz, fmtDb, OCTAVE_Q } from './eq-model.js';
...
export function bandFacts(state) {
    ...
    const b = state[id];
    const def = BANDS[id];
    const hz = id === 'mid' && state.graphic ? snapOctave(b.hz, def.hzMin, def.hzMax) : b.hz;
    const q = id === 'mid' ? (state.graphic ? OCTAVE_Q : b.q) : null;
...
function judgeHpf(f, source, part) {
    const hz = f.hz;
    const slopeLine = f.slope === 24 ? ' The 24 dB slope clears faster and takes less from the octave above; use it when the low end must go.' : ' The 12 dB slope is the gentler, more natural one on a voice.';
    if (source === 'vocal') {
...
export function judge({ state, last = 'preset', part = 'the part' }) {
    ...
    switch (last) {
        ...
        case 'graphic':
            return state.graphic
                ? [seg(3, `Graphic: the Mid band is locked to octave centres (${fmtHz(f.hz)} here) and an octave wide; only its gain moves.`), seg(4, 'Quick to read, and unable to put a narrow cut at 350 Hz: that is the difference the paper asks for. A parametric chooses frequency and width; a graphic only chooses gain.')]
                : [seg(3, 'Parametric: the Mid band is free in frequency, gain and Q.'), seg(4, 'That freedom is the point: one ringing frequency can be cut narrowly and its neighbours left alone. A graphic EQ cannot do that, and the paper credits you for saying so.')];
        case 'match':
            return state.match
    ...
    if (f.id === 'mid') return [name, judgeBell(f, source, part)];
...
export function open({ state, last = 'preset', part = 'the part' }) {
    ...
    switch (last) {
        ...
        case 'slope':
            return `${f.slope} dB/oct is a ${f.slope === 24 ? 'fourth' : 'second'}-order filter: ${f.slope === 24 ? 'two biquad sections in series' : 'one biquad section'}. Each section is 12 dB per octave far from the cutoff and ${f.slope === 24 ? '6 dB down at the cutoff for the pair, the Linkwitz-Riley shape a crossover uses' : '3 dB down at the cutoff, the Butterworth shape'}. The steeper the filter, the more phase it costs around the corner: the dotted line.`;
        case 'q':
...
export function hearingLine(state, part = 'the part') {
    ...
    for (const id of BAND_IDS) {
        const b = state[id];
        if (!b.on) continue;
        const hz = id === 'mid' && state.graphic ? snapOctave(b.hz) : b.hz;
