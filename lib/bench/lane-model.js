...
export const TARGETS = {
    ...
    send: { label: 'Send', lane: 'send → room', dial: 'Send', noun: 'send', said: 'the send', rest: 0, colour: 'var(--gen-6)', node: 'send', ticks: [[1, 'wet'], [0.5, '50%'], [0, 'dry']] },
};
...
export const SONG = {
    ...
    // The bench balance (29 Aug, measured): every part within 3 dB of the
    // others at unity, so a move on any of them is heard. The release has
    // the bass carrying most of the power and the keys 12 dB under the drums.
    mixTrim: { drums: -2, bass: -7, guitar: 13, keys: 11 },
};
...
// Touch: the dial moved at time t while the loop played. A DAW records a
// touch at the automation's own resolution, not the edit grid: here every
// 16th of a beat (36 ms at the groove's tempo). A move within a step
// replaces the point already there, so a passage is one point per step.
export const TOUCH_STEP = 1 / 16;
const touchStep = (t) => clamp(Math.round(t / TOUCH_STEP) * TOUCH_STEP, 0, BEATS);
export function writePoint(state, t, v) {
    const nt = touchStep(t);
    const nv = Math.round(clamp01(v) * 200) / 200;
    const same = state.points.filter((p) => Math.abs(p.t - nt) < TOUCH_STEP / 2 - 1e-9);
    if (same.length === 1 && Math.abs(same[0].v - nv) < 1e-9) return state;
    const kept = state.points.filter((p) => Math.abs(p.t - nt) >= TOUCH_STEP / 2 - 1e-9);
    return { ...state, points: sortPoints([...kept, { t: nt, v: nv }]), presetId: null };
}
// ...and let go at time t: the lane returns to what it was (`before` is
// the state when the touch began) one step later, the way Touch mode
// hands the control back. Off the end of the loop there is nothing to
// return to.
export function touchRelease(state, before, t) {
    const nt = touchStep(t + TOUCH_STEP);
    if (nt >= BEATS - 1e-9) return state;
    const v = valueAt(before.points, before.shape, nt);
    return writePoint(state, nt, v);
}
