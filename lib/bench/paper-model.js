...
// a wave on it. This file is that paper as arithmetic: the sixteen
// questions the page sets, the line the student drew stored as one
// displacement per
...
// The numbers printed under the zero line, one a division: 1 2 3 4 5 on the
// paper's own grid, 2 4 6 8 10 when a division is worth 2 ms. A U6 student,
// 15 Sep 2026, on question 16: the page printed 1 to 5 over a 10 ms grid,
// so a wave drawn at one cycle in the printed 5 ms read as 10 ms and lost
// its mark, and the scheme's dashed answer showed two cycles in "5 ms". The
// numbers now come from the span, and the gate reads them off the canvas.
export const divisionsOf = (span) => Array.from({ length: DIVS }, (_, i) => Math.round(((i + 1) * span) / DIVS * 100) / 100);
...
// ---- the sixteen questions, and the blank paper -----------------------------
// In teaching order: the four waveforms first, drawn from a blank grid; then
// the octave, six questions of it, the last four of which change the shape as
// well as the period ("the ability to understand a waveform and interpret it
// into another waveform, one octave higher or lower", Mike, 12 Sep 2026);
// then louder and inverted; then the papers' own questions in the papers' own
// words. Never two octaves: an octave is one doubling or one halving.
//
// Each question carries what the paper prints (the stem, the figure, the
// marks in brackets), what the grid is worth (timeBase), what the scheme
// wants (want), the scheme's own mark lines (marks) and the examiner report's
// line for the mark most often lost (report). `source` and `part` are the
// teacher's record and are never printed: Mike, 12 Sep 2026, "I don't want to
// see the exam reference ... this is going to tip the students off to which
// exams have this type of question in them". So no year, no paper number and
// no part label reaches the sheet, and both a unit test and the page's gate
// hold the line.
const accept = 'Accept DC offset. Accept different amplitude.';
// Each line is the report's own wording, and each is signed "(examiner's
// report)" rather than with its year: a year on the page tells a student
// which sitting the question came from. The key says the year for the teacher
// reading this file.
const REPORTS = {
    period2023: 'Most candidates drew a square wave but a very common error was a period of 2ms. (examiner\'s report)',
    shape2023: 'A very common error was drawing another square wave, not a saw wave. (examiner\'s report)',
    octave2023: 'Very few candidates knew that an octave lower was double the period. (examiner\'s report)',
    offset2023: 'Many candidates drew the same wave lower on the graph, i.e. with a negative DC offset. (examiner\'s report)',
    period2024: 'Period was often labelled incorrectly as being the width of half a cycle rather than a complete cycle. (examiner\'s report)',
    polarity2024: 'Some of those that did not score had inadvertently drawn a DC offset and failed to mirror it on the graph below. (examiner\'s report)',
    cycle2025: 'Some candidates only drew one cycle of the wave so did not score credit. (examiner\'s report)',
    ms2026: 'A common error was to incorrectly convert from seconds to milliseconds. (examiner\'s report)',
};
...
export const QUESTIONS = [
    ...
    // The four that change the shape as well as the period. Mike, 12 Sep 2026:
    // "giving them an example of a sine wave and saying to them, can you write
    // this waveform now one octave higher ... what we could also do is now
    // switch this up between sine waves, square waves, and triangle waves."
    {
        id: 'triHigher', n: 7, source: 'Practice, in the shape of 2023 Q2(e)(ii)', part: '(e) (ii)',
        stem: 'On the graph below, draw the same wave as in Figure 1, but an octave higher.',
        figure: { shape: 'triangle', periodMs: 2, amp: 0.6, caption: 'Figure 1' }, timeBase: 1,
        want: { shape: 'triangle', periodMs: 1 },
        marks: [shapeMark('triangle'), periodMark(1)],
        accept, report: REPORTS.octave2023,
    },
    {
        id: 'sineToTriLower', n: 8, source: 'Practice, in the shape of 2023 Q2(e)(ii)', part: '(e) (ii)',
        stem: 'On the graph below, draw a triangle wave one octave lower than the wave in Figure 1.',
        figure: { shape: 'sine', periodMs: 1, amp: 0.6, caption: 'Figure 1' }, timeBase: 1,
        want: { shape: 'triangle', periodMs: 2 },
        marks: [shapeMark('triangle'), periodMark(2)],
        accept, report: REPORTS.octave2023,
    },
    {
        id: 'squareToSineHigher', n: 9, source: 'Practice, in the shape of 2023 Q2(e)(ii)', part: '(e) (ii)',
        stem: 'On the graph below, draw a sine wave one octave higher than the wave in Figure 1.',
        figure: { shape: 'square', periodMs: 2, amp: 0.6, caption: 'Figure 1' }, timeBase: 1,
        want: { shape: 'sine', periodMs: 1 },
        marks: [shapeMark('sine'), periodMark(1)],
        accept, report: REPORTS.octave2023,
    },
    {
        id: 'triToSquareLower', n: 10, source: 'Practice, in the shape of 2023 Q2(e)(ii)', part: '(e) (ii)',
        stem: 'On the graph below, draw a square wave one octave lower than the wave in Figure 1.',
        figure: { shape: 'triangle', periodMs: 1, amp: 0.6, caption: 'Figure 1' }, timeBase: 1,
        want: { shape: 'square', periodMs: 2 },
        marks: [shapeMark('square'), periodMark(2)],
        accept, report: REPORTS.octave2023,
    },
    {
        id: 'louder2025', n: 11, source: '2025 Q3(c)(vi)', part: '(c) (vi)',
    ...
    {
        id: 'inverted2024', n: 12, source: '2024 Q4(b), with the figure printed for you', part: '(b)',
    ...
    {
        id: 'lower2023', n: 13, source: '2023 Q2(e)(ii)', part: '(e) (ii)',
    ...
    {
        id: 'lower2025', n: 14, source: '2025 Q3(c)(vii)', part: '(c) (vii)',
        stem: 'On the graph below, draw the same wave as in Figure 1, but an octave lower.',
    ...
    {
        // The 2024 grid is printed with nothing on it at all: no axis names
        // and no division numbers, because labelling the axes is two of the
        // five marks. So this question prints the same bare grid and gives
        // the student two boxes to write the labels in (`blankAxes`).
        id: 'label2024', n: 15, source: '2024 Q4(a)', part: '(a)',
        ...
        figure: null, timeBase: 1, blankAxes: true,
        want: { shape: 'square', periodMs: 'any', offset: 'any' },
        marks: [
            { id: 'shape', words: 'Waveshape (1)', worth: 1, checks: ['shape'] },
            { id: 'axisY', words: 'Voltage / V / displacement (1)', worth: 1, checks: ['axisY'] },
            { id: 'axisX', words: 's / ms / time (1)', worth: 1, checks: ['axisX'] },
            { id: 'amplitude', words: 'amplitude (1). Allow peak to peak amplitude', worth: 1, checks: ['amplitude'], earned: 'labelled for you here; on paper you write these in' },
            { id: 'period', words: 'Period (1)', worth: 1, checks: ['period'], earned: 'labelled for you here; on paper you write these in' },
        ],
        accept: 'Accept DC offsets. The scheme also allows volume, level, amplitude or dB on the vertical axis.',
        report: REPORTS.period2024,
    },
    {
        id: 'kick2026', n: 16, source: '2026 Q1(d), drawn rather than calculated', part: '(d)',
...
// The words the 2024 scheme takes for each axis: "Voltage / V / displacement
// (1)" and "s / ms / time (1)", with the scheme's own allowances ("Allow
// volume / level / amplitude / dB") and pressure, which is the same quantity
// by another name. A label is read as words, so "time in ms" scores and
// "frequency" does not, which is the mistake the 2024 report names.
export const AXIS_WORDS = {
    y: ['voltage', 'volts', 'v', 'displacement', 'amplitude', 'volume', 'level', 'db', 'pressure'],
    x: ['s', 'sec', 'secs', 'second', 'seconds', 'ms', 'millisecond', 'milliseconds', 'time'],
};
export function axisSaid(text, axis) {
    const words = String(text || '').toLowerCase().match(/[a-z]+/g) || [];
    return words.some((w) => AXIS_WORDS[axis].includes(w));
}
...
export function stateAt(i) {
    return { q: clamp(Math.round(i), 0, QUESTIONS.length - 1), line: emptyLine(), checked: false, labels: { y: '', x: '' } };
}
// What the student wrote in an axis box, on the one question that asks for it.
export function setLabel(state, axis, text) {
    if (!(axis === 'y' || axis === 'x')) return state;
    return { ...state, labels: { ...state.labels, [axis]: String(text).slice(0, 40) }, checked: false };
}
...
// Back and Next walk the sixteen and the blank paper. Neither wraps: the
...
const CHECKS = {
    ...
    period: {
        ...
        note: (r, w) => (r.periodMs == null
            ? 'no whole cycle to measure yet'
            : w.periodMs === 'any' || Math.abs(r.periodMs - w.periodMs) / w.periodMs <= PERIOD_TOL
    ...
    axisY: {
        ok: (r, w, state) => axisSaid(state.labels?.y, 'y'),
        note: (r, w, state) => (state.labels?.y
            ? `you wrote "${state.labels.y.trim()}"`
            : 'the vertical axis is not labelled'),
    },
    axisX: {
        ok: (r, w, state) => axisSaid(state.labels?.x, 'x'),
        note: (r, w, state) => (state.labels?.x
            ? `you wrote "${state.labels.x.trim()}"`
...
export function marksFor(state) {
    ...
    return q.marks.map((m) => {
        const failed = m.checks.find((c) => !CHECKS[c].ok(r, q.want, state));
        ...
        return {
            ...
            worth: m.worth || 1,
            ok: !failed,
            // a mark the page earns for the student says so in its own words
            note: !failed && m.earned ? m.earned : CHECKS[said].note(r, q.want, state),
        };
    });
}
