...
export const OSC2_IDS = ['pair', 'sub', 'off'];
export const OSC2 = {
    pair: { label: 'Pair', said: 'a second oscillator of the same wave, detuned against the first' },
...
export function gateMs(state, bpm = BPM) { const p = PARTS[state.part]; return Math.round(p.notes[0].len * stepSec(bpm) * p.gate * 1000); }
...
export function judgeSection(state, section, part = state.part) {
    ...
    if (section === 'osc') {
        ...
        if (part === 'pad') {
            ...
            return V('good', `${WAVES[w].said} ${s.osc2 === 'pair' ? `in a pair ${s.detune} cents apart: width and slow movement` : 'with a sub beneath: weight'}`);
        }
        if (part === 'lead') {
        ...
        if (w === 'saw') return V('good', `a saw${s.osc2 === 'pair' ? ` pair, ${s.detune} cents apart` : ''}: the 2022 and 2024 keyboard parts are two saws`);
...
export function schemePoints(state) {
    ...
    if (t === 'as2023') {
        return [
            ...
            P('detune', 'detune, a suitable amount', s.osc2 === 'pair' && s.detune >= 4 && s.detune <= 30, s.osc2 !== 'pair' ? 'no second oscillator to detune' : s.detune < 4 ? 'no detune to speak of' : s.detune > 30 ? `${s.detune} cents apart, past detune into out of tune` : `${s.detune} cents apart`),
    ...
    if (t === 'as2024') {
        return [
            ...
            P('detune', 'slight detune', s.osc2 === 'pair' && s.detune >= 3 && s.detune <= 15, s.osc2 !== 'pair' ? 'nothing to detune' : s.detune < 3 ? 'no detune' : s.detune > 15 ? `${s.detune} cents is more than slight` : `${s.detune} cents, slight`),
