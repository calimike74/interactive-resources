...
// with a source mixer (pulse, saw, sub, noise) into a filter into an
...
export const WAVE_IDS = ['square', 'pulse', 'saw', 'noise', 'none'];
export const WAVES = {
...
// RMS sits a little under the others). The same numbers scale the bars on
// the stage.
export const SOURCE_GAIN = { pulse: 0.28, saw: 0.55, sub: 0.25, noise: 0.3 };
...
export function sources(s) {
    ...
    if (s.saw > 0) out.push({ id: 'saw', level: s.saw, said: 'a saw wave' });
...
export function sourcesShort(s) {
    ...
    return list.map((x) => (x.id === 'pulse' ? (isSquare(s) ? 'square' : 'pulse') : x.id)).join(' + ');
}
export function harmonicsSaid(s) {
    ...
    if (s.saw > 0) bits.push('saw: every harmonic, 1/n');
...
function baseState() {
    return {
        part: 'bass',
        pulse: 100, saw: 0, sub: 0, noise: 0, width: 50, pwm: 'man', subOct: 1,
...
export function spectrum(state, f0 = midiHz(homeMidi(state)), { hi = HZ_HI, n = N_HARMONICS, width = null } = {}) {
    ...
    const gS = (state.saw / 100) * SOURCE_GAIN.saw * pair;
    const series = (f, osc) => {
        for (let k = 1; k <= n; k += 1) {
            const ap = gP * harmonicAmp('pulse', k, w); const as = gS * harmonicAmp('saw', k);
...
export function waveShape(state, { n = 256, cycles = 2, harmonics = N_HARMONICS, width = null } = {}) {
    ...
    const vco = (f) => {
        const gP = (state.pulse / 100) * SOURCE_GAIN.pulse * pair; const gS = (state.saw / 100) * SOURCE_GAIN.saw * pair;
        for (let k = 1; k <= harmonics; k += 1) { if (gP) tone(f, k, gP * pulseCoef(k, w), 0, true); if (gS) tone(f, k, gS * harmonicAmp('saw', k), 0, false); }
    };
    vco(1);
...
export function schemePoints(state) {
    ...
    if (t === 'a2025') {
        return [
            P('wave', 'square (allow saw, pulse, triangle; not sine)', w !== 'none' && w !== 'noise', w === 'none' ? 'no source in the mixer' : w === 'noise' ? 'noise alone, which has no pitch' : sourceSaid(s)),
