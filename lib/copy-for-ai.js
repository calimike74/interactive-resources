...
export function buildAdditiveCopyMarkdown({ amps, signs, quizResults, mode = 'daw', learnMode = false }) {
    const lines = [];

    lines.push('# Context');
    lines.push("I'm an A-Level Music Technology student.");
    lines.push("I'm building a sound with additive synthesis in an interactive tool — stacking a fundamental and its harmonics, each at a chosen level.");
    lines.push('');

    lines.push('# My Harmonic Recipe');
    amps.forEach((amp, i) => {
        const n = i + 1;
        if (amp <= 0) {
            lines.push(`- Harmonic ${n}: silent`);
            return;
        }
        const inverted = signs && signs[i] < 0 ? ', inverted (phase flipped)' : '';
        lines.push(`- Harmonic ${n}: ${Math.round(amp * 100)}%${inverted}`);
    });
    lines.push('');
    ...
    lines.push('# What I need help with');
    lines.push(getProductionPrompt(mode, 'additive synthesiser', learnMode));
