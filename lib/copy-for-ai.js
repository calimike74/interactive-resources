...
function getLearnProductionPrompt(mode, toolName) {
    switch (mode) {
        case 'ableton':
            return `Before showing me, ask me which Ableton plugin I'd use for this ${toolName} and where each parameter maps. Guide me through finding the right settings myself.`;
        case 'explain':
 ...
 */
export function buildCompressorCopyMarkdown({ threshold, ratio, attack, release, knee, makeupGain, source, presetName, quizResults, mode = 'ableton', learnMode = false }) {
    const lines = [];
 ...
 */
export function buildEQCopyMarkdown({ bands, gains, presetName, quizResults, mode = 'ableton', learnMode = false }) {
    const lines = [];
 ...
 */
export function buildSynthCopyMarkdown({ waveform, filterType, cutoff, resonance, attack, decay, sustain, release, octave, presetName, quizResults, mode = 'ableton', learnMode = false }) {
    const lines = [];
    ...
    lines.push('# What I need help with');
    lines.push(getProductionPrompt(mode, 'synthesizer', learnMode));
