...
/**
 * Split a canonical label (e.g. "1.12 Reverb") into its number and name.
 *
 * This is how lib/topics.js derives each band's displayed number + name
 * instead of retyping them — a band stores one `specTopic` string, and this
 * function is the only thing allowed to split it. Throws on anything that
 * isn't an exact SPEC_TOPICS or NON_SPEC_TOPICS entry, so a typo fails the
 * build loudly instead of drifting onto the page silently (the defect this
 * file exists to prevent — see the file header).
 */
const CANON_LABELS = new Set([...SPEC_TOPICS, ...NON_SPEC_TOPICS]);

export function splitSpecLabel(label) {
    if (!CANON_LABELS.has(label)) {
        throw new Error(
            `"${label}" is not an exact entry in SPEC_TOPICS or NON_SPEC_TOPICS. ` +
            `lib/topics.js bands derive their display data from this file by design — ` +
            `fix the label here, don't retype a variant in lib/topics.js.`
        );
    }
    const spaceIndex = label.indexOf(' ');
    return { number: label.slice(0, spaceIndex), name: label.slice(spaceIndex + 1) };
}
