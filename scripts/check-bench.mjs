    ...
    for (const size of SIZES) {
        ...
        // the bench plays, and at every level the prose fits the line
        {
            ...
            const readNote = () => page.evaluate(() => {
                ...
                return prose ? { left: Math.round(prose.getBoundingClientRect().left * 10) / 10, over: prose.scrollWidth - prose.clientWidth, text: prose.textContent.trim() } : null;
            });
            for (const lv of levels) {
                if (lv) { await depthBtn(lv).click(); await page.waitForTimeout(250); }
                const lefts = new Set(); let over = 0; let seen = false; let text = '';
                for (let i = 0; i < 6; i += 1) {
                    const r = await readNote();
                    if (r) { seen = true; lefts.add(r.left); over = Math.max(over, r.over); text = r.text; }
                    await page.waitForTimeout(220);
                }
                const name = lv ? lv.toLowerCase() : 'the stage';
                if (!seen) { fail(url, size, 'no stage note to read'); break; }
                // 12 Sep 2026: an orientation sentence keyed by the stage's name
                // rather than the level's came out blank at Core, and the law
                // read an empty box as a still one.
                if (!text) fail(url, size, `the stage note has no orientation sentence at ${name}`);
                else if (lefts.size > 1) fail(url, size, `the stage note moves while the bench plays at ${name} (left edge at ${[...lefts].join(', ')})`);
