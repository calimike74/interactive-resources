...
function shapeToADSR(shape) {
    const t = shape / 100;
    const lerp = (a, b) => a + (b - a) * t;
    return {
        attack: lerp(0.005, 1.4),
        decay: lerp(0.35, 0.2),
        sustain: lerp(0, 0.85),
        release: 0.2,
    };
...
export default function ADSRShaper() {
    ...
    const start = useCallback(() => {
        ...
        handleRef.current = startPreset('ctl-adsr');
        handleRef.current.set(shapeToADSR(shape));
        setPlaying(true);
    }, [shape]);
    useEffect(() => stop, [stop]);
    ...
    const onChange = (e) => {
        ...
        setShape(value);
        handleRef.current?.set(shapeToADSR(value));
    };
    ...
    return (
        <div style={{
            ...
            <input
                type="range" min="0" max="100" step="1" value={shape}
                onChange={onChange}
                aria-label="Envelope shape, plucky to swelling"
