...
export default function DriveSlider() {
    ...
    const start = useCallback(() => {
        ...
        handleRef.current = startPreset('ctl-drive');
        handleRef.current.set({ drive });
        setPlaying(true);
    }, [drive]);
    useEffect(() => stop, [stop]);
    ...
    const onChange = (e) => {
        ...
        setDrive(value);
        handleRef.current?.set({ drive: value });
    };
    ...
    return (
        <div style={{
            ...
            <input
                type="range" min="0" max="10" step="1" value={drive}
                onChange={onChange}
                aria-label="Drive amount, 0 to 10"
            ...
            <button
                ...
                aria-label={playing ? 'Release to stop the sustained tone' : 'Hold to hear the sustained tone'}
                style={{
